...
const page = ({params}) => {
    console.log(params);
    const requiredBlog = blogs.find(blog => blog.slug == params?.slug);

    return (
        <div className='border-2 border-red-300 p-4 h-screen'>
            <h1 className='font-semibold mb-2'>{requiredBlog?.title}</h1>
            <h1>{requiredBlog?.description}</h1>
        </div>
    );
};
